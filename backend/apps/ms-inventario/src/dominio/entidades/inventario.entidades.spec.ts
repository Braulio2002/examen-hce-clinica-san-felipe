import { LineaCompra, LineaVenta, ReglasDocumento } from './inventario.entidades';
...
describe('ReglasDocumento', () => {
  ...
  describe('validarLineasCompra', () => {
    it('acepta un documento con lineas correctas', () => {
      expect(() => ReglasDocumento.validarLineasCompra([lineaCompraValida])).not.toThrow();
    });

    it('rechaza una compra sin lineas', () => {
      expect(() => ReglasDocumento.validarLineasCompra([])).toThrow(
        /al menos un producto/i,
      );
    });

    it('rechaza una compra que supera el maximo de lineas permitido', () => {
      const demasiadas = Array.from({ length: ReglasDocumento.MAX_LINEAS + 1 }, (_, i) => ({
        ...lineaCompraValida,
        idProducto: i + 1,
      }));

      expect(() => ReglasDocumento.validarLineasCompra(demasiadas)).toThrow(/lineas de detalle/i);
    });
  ...
  describe('validarLineasVenta', () => {
    it('acepta un documento con lineas correctas', () => {
      expect(() => ReglasDocumento.validarLineasVenta([lineaVentaValida])).not.toThrow();
    });

    it('rechaza una venta sin lineas', () => {
      expect(() => ReglasDocumento.validarLineasVenta([])).toThrow(/al menos un producto/i);
    });
  ...
  describe('totalesPrevistos', () => {
    it('coincide con la formula del enunciado', () => {
      ...
      expect(totales.igv).toBe(159.3);
      expect(totales.total).toBe(294.3);
    });
  });
});
